...
it("reveals password and submits Enter without confirmation field", async () => {
  vi.mocked(signIn).mockResolvedValue({ user: { id: "1", email: "person@example.com", created_at: "now" } });
...
it("loads a verified user and signs out", async () => {
  vi.mocked(session).mockResolvedValue({ user: { id: "1", email: "person@example.com", created_at: "now" } }); vi.mocked(signOut).mockResolvedValue({} as never);
  render(<Dashboard />); expect(await screen.findByText("person@example.com")).toBeVisible(); await userEvent.click(screen.getByRole("button", { name: "Sign out" })); await waitFor(() => expect(router.replace).toHaveBeenCalledWith("/login"));
});
it("shows loading, then redirects an unauthenticated user", async () => {
  let reject!: (e: Error) => void; vi.mocked(session).mockReturnValue(new Promise((_, no) => { reject = no; })); render(<Dashboard />); expect(screen.getByRole("status")).toBeVisible(); await act(async () => reject(new AuthError("Sign in", 401))); expect(router.replace).toHaveBeenCalledWith("/login");
});
it("keeps a retry action after service outage and pauses automatic retry", async () => {
  vi.mocked(session).mockRejectedValue(new AuthError("Service unavailable", 503)); render(<Dashboard />); expect(await screen.findByRole("alert")).toHaveTextContent("Service unavailable"); fireEvent.focus(window); expect(session).toHaveBeenCalledTimes(1); await userEvent.click(screen.getByRole("button", { name: "Try again" })); expect(session).toHaveBeenCalledTimes(2);
});
it("reacts to a session change in another tab", async () => {
  let changed!: () => void; vi.mocked(subscribeAuth).mockImplementation(listener => { changed = listener; return () => {}; }); vi.mocked(session).mockResolvedValue({ user: { id: "1", email: "person@example.com", created_at: "now" } }); render(<Dashboard />); await screen.findByText("person@example.com"); vi.mocked(session).mockRejectedValueOnce(new AuthError("Ended", 401)); await act(async () => changed()); expect(router.replace).toHaveBeenCalledWith("/login");
});
